...
> **2026-09-10 整理说明：** 本文保留产品范围和验收契约；历史修订记录不表示今日实施状态。OpenArm 是当前适配目标，MQ03 是源数据解释材料，Panda 是已有回归夹具。当前暂停状态、报告口径和真实进度见 [current-status.md](current-status.md)，实验引用见 [report-index.md](report-index.md)。

> **文档状态：现行唯一产品基线（2026-09-02 第七轮）。** 本文取代 [`current-product-plan.md`](archive/product-plan-v1.md)；后者转为历史归档。产品范围以本文为准，代码结构与验收细节以 [`engineering-plan-v1.md`](engineering-plan-v1.md) v1.2 为准；若发现真实冲突，先修文档再编码，不静默选择其一。
>
...
> **当前执行覆盖（2026-09-04）：OpenArm-first。** 用户已确认本工作区优先
> 完成 OpenArm 的语义、单臂与双臂数值闭环。文中早期的 “M0 Panda →
> M1 OpenArm” 是设计演进记录；当前执行顺序由 current-status / build-checklist 明确为
> “OpenArm M-1 语义/约束闸门 → OpenArm 正式基础 → M1a/b/c”。Panda 只
> 保留为历史目标重选证据和后续回归夹具，不能复用其映射或可达结论。
...
3. **必须做一次双臂可达性预检，这是原文漏掉的、可能推翻目标机器人选择的检查。** §2.2 实测源数据 `x ∈ [0.16, 0.61]`、`z ∈ [0.57, 0.90]`，左右臂 `y` 合计跨度约 1 m。OpenArm 双臂配置**是否覆盖得住这个范围，目前没有任何证据**。
   ...
   **但预检不能只比包围盒（2026-08-27 修正）。** 「源轨迹 AABB 落在可达点云的包围范围内」只是一个很弱的必要条件，它证明不了姿态可达、证明不了两臂能同时到位、更证明不了连续轨迹能保持在同一分支。正确的预检是**直接拿真实数据抽样跑 IK**：
   ...
   | 检查 | 做法 |
   |---|---|
   | 位置 + 姿态可达 | 只从 calibration 12 个 episode 分层抽样（含轨迹端点与极值帧），带完整位姿求 IK，不只查位置 |
   ...
   这比包围盒贵，但仍然是几小时量级，且**它产出的就是 M1 要用的 T2 摆位**，不是一次性的丢弃工作。若结果是覆盖不住，要动的是目标机器人选择或 T2 摆位——**正式使用私有数据闭环前必须明确这个结论**；已有合成基础代码并不替代 OpenArm 的可行性闸门。

通用基础已使用 Panda 与合成夹具开展测试；当前继续的是 OpenArm 私有数据适配，不重做基础、不把 Panda 测试分数迁移给 OpenArm。阶段出口仍须分别验证。

以下是历史开工前的后续后端调研项，不是当前 OpenArm CPU 适配的阻塞项：CuRobo `content/configs/robot/` 现成有哪些配置；CuRobo 当前版本对多末端 / 人形的支持程度；MuJoCo Menagerie 里目标机器人是否齐全（OpenArm 已可由官方 `openarm_mujoco` 覆盖）。

---

## 12. 里程碑

### M-1 可行性闸门（Feasibility Gate，先于目标私有数据的正式闭环）

2026-09-10 勘误：下面保留 harness 与正式管线的设计分工。已有 M0/M1a/M1b 代码切片不会因 OpenArm gate 未过而消失，也不能据其存在宣布该目标的数据验收完成。
...
| **M0** | 通用后端；Panda 等合成/回归夹具 | 正式通用管线与夹具碰撞资产；已有实现不重复建设 | 成为基线代码 |
| **M1** | OpenArm | 将通过验证的映射接入已有正式 Profile、碰撞和双流管线，完成私有数据出口 | 正式交付 |
...
- **任一项红灯 →** 当前 recipe 记录 RED，按明确失败原因处理映射、求解或有方向的 T2 校准；修改须新 recipe。当前已确认 OpenArm-first，不自动转向 Panda，也不静默放宽任务。历史目标重选方案只作记录。

**历史目标重选路线（不自动启动）：** Franka Panda 双臂曾有独立 YELLOW 实验记录，详见报告索引；它不能替代 OpenArm 验证。原设计要求保留如下： 它虽然复用 M0 的单臂 Panda 资产，但双臂 root、两臂 base 相对变换、双夹爪/TCP、臂间碰撞对和同时可达性都仍需单独建立与验证。选择它或其他候选后，必须重新执行 M-1 的完整资产核查、抽样和红黄绿判据，不得沿用 OpenArm 的结果；没有候选通过前，M1 保持阻塞。
...
### M0 通用骨架与合成闭环（纯 CPU；Panda 为已有回归夹具）

- `CanonicalTrajectory v0.1`（标记 provisional，M1c 出口才冻结结构）
- Pinocchio 集成：URDF 加载、FK、Jacobian
- 回归夹具的 `RobotProfile`、`GeometryModel` 与碰撞对屏蔽表（§4.3）；已有 Panda 实现不改变 OpenArm-first 交付顺序
- §10.1 合成往返测试台 + 负向样例生成
- Pink 求解后端
...
本次不扩大范围、不改 nominal/relaxed 阈值。将 OpenArm-first 执行状态、已实现代码切片和未通过数据验收分开；恢复 9 月 9 日未同步的 position-only 95%/90%/85% 与 full-pose 78.33%/88.33% 报告。当前状态由 current-status 管理，旧日志与 blocker 保留为历史。用户已要求停止实验，本次只整理。

早期全 20 ep 数值分析记录与后设 calibration/held_out 划分存在待审计关系；不能以某次运行的 held_out=false 声明证明历史独立性。此项在 M1c 前以现有资料审计，不重新读取留出数值。
