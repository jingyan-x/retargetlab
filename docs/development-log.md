...
| 恢复后完整套件 | 144 passed / 1 skipped | 两资产均启用，唯一skip为未安装LeRobot |
| 恢复后 world/tool 与 gate 测试 | 16 passed | 代码约束验证，不是轨迹通过率 |
| 资料整理 | 28个run、377个文件、129份日志；指纹与关键指标核对通过 | 原始运行产物保留，未重新执行实验 |
